...
import { PhuketMapData } from '../data/culture'

export default function PhuketMap() {
  const [activeDay, setActiveDay] = useState(0)
  const current = PhuketMapData[activeDay]

  return (
    <div id="phuket-map" className="mb-10 scroll-mt-20">
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50">
            ...
            <div className="absolute top-4 left-4 flex items-center gap-2">
              <span className="text-xs font-bold px-3 py-1.5 rounded-full text-white shadow-md" style={{ background: current.color }}>
                {current.day === 0 ? '🗺️ 全景' : current.day === 5 ? '🏝️ 跳岛' : `DAY ${current.day}`}
              </span>
              {current.spots > 0 && (
          ...
          <div className="p-5 border-t border-gray-100">
            ...
            <div className="mt-3 h-1.5 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all duration-500"
                style={{ width: `${((activeDay + 1) / PhuketMapData.length) * 100}%`, background: current.color }} />
            </div>
          </div>
        </div>
      </div>
